...
export function ConsultantHeader({
  ...
  return <div className="px-6 md:px-10 pb-8 my-0">
      <div className="flex flex-col md:flex-row gap-6 -mt-16">
        <Avatar className="h-32 w-32 border-4 border-white bg-white shadow-lg">
          {consultant.profile_picture_url ? <AvatarImage src={consultant.profile_picture_url} alt={consultant.name || "Consultant"} className="object-fill" /> : <AvatarFallback className="text-3xl bg-primary/10">{getInitials()}</AvatarFallback>}
        </Avatar>
        <div className="pt-4 md:pt-0">
          <h1 className="text-3xl md:text-4xl font-bold mb-2 font-lora">{consultant.name || "Consultant"}</h1>
          <p className="text-lg text-gray-600 mb-4 italic my-[33px]">
            {generateTagline()}
          </p>
          <div className="flex flex-wrap gap-2 mb-4">
            ...
            {consultant.experience && consultant.experience > 0 ? <Badge variant="outline" className="text-sm px-3 py-1">
                {consultant.experience} Years Experience
              </Badge> : null}
            {consultant.is_available ? <Badge variant="outline" className="bg-green-50 text-green-700 border-green-200 text-sm px-3 py-1">
                Available
              </Badge> : <Badge variant="outline" className="bg-gray-50 text-gray-700 border-gray-200 text-sm px-3 py-1">
