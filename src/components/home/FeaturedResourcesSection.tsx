    ...
    <section className="py-16 bg-softGray">
      <div className="container mx-auto px-4">
        ...
        <div className="mt-12 text-center">
          <Link 
            to="/resources" 
            className="inline-block rounded-full px-6 py-3 border border-peacefulBlue text-peacefulBlue hover:bg-peacefulBlue/5 transition-colors"
          >
            View All Resources
          </Link>
        </div>
...
const ResourceCard = ({ resource }: ResourceCardProps) => {
  const { category, title, summary, gradient, isComingSoon } = resource;

  return (
    <SiteCard className="flex flex-col h-full hover:shadow-md transition-all duration-300 overflow-hidden group">
      ...
      <div className="p-6 flex flex-col flex-grow">
        ...
        {isComingSoon ? (
          ...
          <Link 
            ...
            className="flex items-center gap-1 text-peacefulBlue hover:text-peacefulBlue/90 font-medium"
          >
            <span>Read More</span>
            <BookOpen className="h-4 w-4" />
          </Link>
