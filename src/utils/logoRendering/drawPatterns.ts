...
/**
 * Draws Da Vinci-inspired human figure lines extending from center
 */
export const drawVitruvianLines = (
  ctx: CanvasRenderingContext2D, 
  centerX: number, 
  centerY: number, 
  radius: number, 
  ...
  // Define the angles for the four limbs (in radians)
  // 45°, 135°, 225°, and 315° converted to radians
  ...
  // For each angle, draw a limb line
  angles.forEach((angle, index) => {
    // Calculate end point
    const lineLength = radius * 1.5 * scale;
    const endX = centerX * scale + Math.cos(angle) * lineLength;
    const endY = centerY * scale + Math.sin(angle) * lineLength;
    ...
    // Add dotted style to make it look like measurement lines
    ctx.setLineDash([scale * 2, scale * 1.5]); 
    ctx.stroke();
    
    // Add small circles at certain points along the line to mimic
    // the proportion marks in Da Vinci's drawing
    const divPoints = 3;
    for (let i = 1; i <= divPoints; i++) {
      const pointX = centerX * scale + Math.cos(angle) * (lineLength * i/divPoints);
      const pointY = centerY * scale + Math.sin(angle) * (lineLength * i/divPoints);
      
      // Draw small circle at division point
      ctx.beginPath();
      ctx.arc(pointX, pointY, scale * 0.8, 0, Math.PI * 2);
      ctx.fillStyle = color;
      ctx.globalAlpha = 0.6;
      ctx.fill();
    }
