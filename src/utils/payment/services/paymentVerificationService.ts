...
export async function verifyRazorpayPayment(
  ...
  try {
    ...
    // Use the payment-status endpoint with GET request
    const { data, error } = await supabase.functions.invoke('payment-status', {
      body: null, // No body for GET request
      method: 'GET'
    });
    
    // Construct the URL for payment-status endpoint
    ...
    // Handle different status cases according to spec
    if (responseData.success && responseData.status === 'captured') {
    ...
    } else if (responseData.status === 'pending_webhook' || responseData.status === 'not_found') {
      // These are expected states during payment processing
      return { success: false, status: responseData.status };
    } else if (responseData.status === 'failed') {
      return { success: false, status: 'failed', error: 'Payment failed' };
    } else {
      return { success: false, status: 'error', error: responseData.reason || 'Unknown error' };
    }
  } catch (error) {
